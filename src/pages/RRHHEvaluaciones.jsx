...
export default function RRHHEvaluaciones() {
  ...
  const [viewMode, setViewMode] = useState("evaluaciones"); // "evaluaciones" | "feedbacks"
  const [comentarioRRHH, setComentarioRRHH] = useState(""); // Comentario opcional al cerrar
  ...
  const loadPending = async () => {
    ...
    try {
      ...
      if (viewMode === "evaluaciones") {
        ...
        endpoint = `/evaluaciones/hr/pending?${qs.toString()}`;
      } else {
        // Feedbacks endpoint
        // year is required for feedbacks usually, but let's assume backend handles optional or we extract from period
        if (periodo && periodo.length >= 4) {
          const y = periodo.substring(0, 4);
          if (!isNaN(y)) qs.set("year", y);
        }
        endpoint = `/feedbacks/hr/pending?${qs.toString()}`;
      }

      const res = await api(endpoint);
      console.log(`[RRHH] ${endpoint} raw =>`, res);
  ...
  const closeSelected = async () => {
    ...
    try {
      const endpoint = viewMode === "evaluaciones" ? "/evaluaciones/hr/close-bulk" : "/feedbacks/hr/close-bulk";
      await api(endpoint, {
        method: "POST",
        body: { ids: Array.from(checked), comentarioRRHH },
      });
      toast.success("Cerradas.");
      setComentarioRRHH(""); // Limpiar tras cerrar
      await loadPending();
    } catch (e) {
      console.error(e);
  ...
  const closeAll = async () => {
    ...
    try {
      const endpoint = viewMode === "evaluaciones" ? "/evaluaciones/hr/close-bulk" : "/feedbacks/hr/close-bulk";
      // Nota: closeAll para feedbacks no soporta filtro complejo aun en backend, pero si pasamos IDs filtrados seria mejor.
      // Por simplicidad, asumimos que el backend soporta filtro o enviamos IDs.
      // Dado que pending ya tiene los items filtrados, mejor enviamos IDs de todo pending.
      const allPendingIds = pending.map(p => String(p._id));
      ...
      toast.success("Cerradas todas las visibles.");
      setComentarioRRHH("");
      await loadPending();
    } catch (e) {
      console.error(e);
    ...
    <div className="container-app space-y-6">
      ...
      <div className="rounded-xl bg-card shadow-sm ring-1 ring-border/60 p-4">
        <div className="flex items-center gap-4 mb-4 border-b pb-4">
          <h2 className="text-lg font-bold text-slate-800">Cierre de RRHH</h2>
          <div className="flex bg-slate-100 p-1 rounded-lg">
            <button
              onClick={() => setViewMode("evaluaciones")}
              className={`px-3 py-1 text-sm font-medium rounded-md transition-all ${viewMode === "evaluaciones" ? "bg-white shadow text-blue-600" : "text-slate-500 hover:text-slate-700"}`}
            >
              Evaluaciones de Objetivos
            </button>
            <button
              onClick={() => setViewMode("feedbacks")}
              className={`px-3 py-1 text-sm font-medium rounded-md transition-all ${viewMode === "feedbacks" ? "bg-white shadow text-blue-600" : "text-slate-500 hover:text-slate-700"}`}
            >
      ...
      {viewMode === "feedbacks" ? (
        ...
        <>
          ...
          {/* Comentario RRHH Input */}
          <div className="mt-4 pt-4 border-t border-border/50">
            <label className="text-xs font-medium text-slate-500 mb-1 block">Comentario de Cierre (Opcional)</label>
            <div className="flex gap-2">
              <input
                className="flex-1 rounded-md border border-border bg-background px-3 py-2 text-sm"
                placeholder="Escribí un comentario para guardar al cerrar..."
          ...
          {Object.entries(grouped).map(([grupo, rows]) => (
            <div
              key={grupo}
              className="mt-6 rounded-xl bg-card shadow-sm ring-1 ring-border/60 overflow-hidden"
            >
